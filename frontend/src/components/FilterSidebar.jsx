...
const FilterSidebar = ({ filters, onFilterChange, onClearFilters, isOpen, onClose }) => {
    ...
    return (
        <div className={`bg-white rounded-lg shadow-md overflow-hidden ${isOpen ? 'block' : 'hidden'} lg:block`}>
            {/* Header */}
            ...
            <div className="p-4 max-h-[calc(100vh-200px)] overflow-y-auto space-y-4">
                ...
                <FilterSection title="Experience" sectionKey="experience">
                    {filterOptions.experience_ranges.map((range) => (
                        <label key={range.label} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-2 rounded">
                            <input
                                type="radio"
                                name="experience"
                ...
                <FilterSection title="Rating" sectionKey="rating">
                    {filterOptions.ratings.map((ratingOption) => (
                        <label key={ratingOption.label} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-2 rounded">
                            <input
                                type="radio"
                                name="rating"
                                ...
                                className="w-4 h-4 text-blue-600 focus:ring-blue-500"
                            />
                            <span className="text-sm text-gray-700">{ratingOption.label}</span>
                        </label>
                    ))}
                </FilterSection>
                ...
                <FilterSection title="Other Filters" sectionKey="other">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-2 rounded">
                        ...
                        />
                        <span className="text-sm text-gray-700">Verified Lawyers Only</span>
                    </label>
                    ...
                    <div className="mt-3">
                        ...
                        {filterOptions.genders.map((gender) => (
                            <label key={gender} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-2 rounded">
                                <input
                                    type="radio"
                                    name="gender"
            ...
            <div className="p-4 border-t border-gray-200 bg-gray-50">
                <button
                    ...
                    className="w-full px-4 py-2 text-sm font-medium text-blue-600 bg-white border border-blue-600 rounded-lg hover:bg-blue-50 transition-colors"
                >
                    Clear All Filters
                </button>
            </div>
