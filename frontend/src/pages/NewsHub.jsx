    ...
    <div className="flex flex-1 flex-col gap-6">
      ...
      <div className="flex flex-col gap-6 lg:flex-row">
        ...
        <aside className="w-full lg:w-64 flex-shrink-0 space-y-6">
          {/* Navigation Tabs */}
          <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-2 shadow-sm">
            <button
              onClick={() => {
                setActiveTab('news');
              ...
              {currentCategories.map(cat => (
                <button
                  ...
                  className={`rounded-md px-3 py-2 text-left text-xs font-medium transition-colors ${categoryFilter === cat || (cat === 'All' && !categoryFilter)
                    ...
                    : 'bg-slate-50 dark:bg-slate-950 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:bg-slate-800'
                    }`}
                >
                  {cat}
                </button>
        ...
        <section className="flex-1">
          {loading ? (
            ...
            <div className="flex h-64 flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-slate-50 dark:bg-slate-950 text-slate-500 dark:text-slate-400">
            ...
            <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
              {items.map((item) => (
                activeTab === 'news' ? (
                  // News Card
                  <article key={item.id} className="group relative flex flex-col rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm transition-all hover:shadow-md">
                    <div className="p-5 flex flex-col h-full">
                      ...
                      <h3 className="mb-2 text-lg font-bold text-slate-900 dark:text-white line-clamp-2 md:text-xl group-hover:text-blue-600">
                        {item.title}
                      </h3>

                      <p className="mb-4 flex-1 text-sm text-slate-600 dark:text-slate-400 line-clamp-3 leading-relaxed">
                      ...
                      <div className="mt-auto flex items-center justify-between border-t border-slate-100 dark:border-slate-800 pt-4">
                        <div className="flex items-center gap-2 text-xs font-medium text-slate-700 dark:text-slate-300">
                          <span className="h-6 w-6 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase">
                  ...
                  // Video Card
                  <div key={item.id} className="overflow-hidden rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm transition-all hover:shadow-md">
                    {/* YouTube Embed */}
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 mt-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium text-slate-700 dark:text-slate-300">{item.channel_name}</span>
                        </div>
        ...
        <div className="rounded-[15px] bg-gradient-to-r from-slate-900 via-blue-900 to-blue-600 p-8 md:p-12 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <div className="relative z-10 flex flex-wrap items-center justify-center gap-4">
            <button
              ...
              className="px-8 py-3 bg-[#FCB827] hover:bg-[#EAA616] text-slate-900 dark:text-white font-bold rounded-full shadow-lg shadow-yellow-500/20 transform transition-transform hover:scale-105"
            >
      ...
      {/* Disclaimer Section */}
      <footer className="mt-8 rounded-lg bg-slate-50 dark:bg-slate-950 p-4 border border-slate-200 dark:border-slate-700 text-center">
