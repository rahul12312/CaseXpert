...
const CaseTracker = () => {
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-4 sm:p-6 bg-slate-50 min-h-screen">
      {/* Header */}
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Case Tracker</h1>
          <p className="text-slate-500 font-medium">Single source of truth for your legal proceedings.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatsCard title="Total Repository" value={stats.total} color="bg-white border-l-4 border-blue-500 shadow-sm" textColor="text-blue-600" />
        <StatsCard title="Active Files" value={stats.active} color="bg-white border-l-4 border-emerald-500 shadow-sm" textColor="text-emerald-600" />
        <StatsCard title="Upcoming Hearings" value={stats.upcoming_hearings} color="bg-white border-l-4 border-amber-500 shadow-sm" textColor="text-amber-600" />
        <StatsCard title="Security" value="Encrypted" color="bg-white border-l-4 border-violet-500 shadow-sm" textColor="text-violet-600" isText />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
        ...
        <select 
          ...
          className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none"
        >
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-xl overflow-hidden">
        <div className="border-b border-slate-200 px-6 py-4 bg-slate-50/50 flex justify-between items-center">
          <h2 className="text-lg font-bold text-slate-900">Litigation Records</h2>
        ...
        <div className="overflow-x-auto">
          {loading ? (
            <div className="p-20 text-center flex flex-col items-center gap-4">
              ...
              <p className="text-slate-500 font-medium">Synchronizing with legal vault...</p>
            </div>
            ...
            <div className="p-20 text-center">
              ...
              <p className="text-slate-500 text-lg font-medium">No cases found matching your criteria.</p>
                ...
                {filteredCases.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/80 transition-colors group">
                    <td className="px-6 py-5">
                      <div className="font-bold text-slate-900 group-hover:text-primary-700 transition-colors">{item.title}</div>
                      <div className="text-[11px] font-bold text-slate-400 mt-1 uppercase tracking-tighter">REF: {item.case_number}</div>
                    </td>
                    <td className="px-6 py-5">
                      <span className="capitalize font-medium px-2 py-1 bg-slate-100 rounded text-slate-600 border border-slate-200">
                        {item.case_type}
                      </span>
                    </td>
                    <td className="px-6 py-5">
