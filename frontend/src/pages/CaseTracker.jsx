...
const CaseTracker = () => {
  ...
  useEffect(() => {
    ...
    const loadCases = async () => {
      ...
      try {
        ...
        const caseList = response.data.data || response.data.cases || [];
  ...
  const handleClearSamples = async () => {
    if (!window.confirm('This will permanently delete ALL duplicate "Sample Property Dispute" cases. Your real cases will not be affected. Continue?')) return;
    try {
      const res = await api.delete('/case/clear-samples');
      if (res.data.success) {
        // Remove sample cases from local state immediately
        setCases(prev => prev.filter(c =>
          !(c.title === 'Sample Property Dispute' && c.description === 'Automated sample case created via seeder.')
        ));
        alert(res.data.message);
      }
    } catch (err) {
      alert('Failed to clear sample cases.');
    }
  };

  // Detect if any auto-generated sample cases exist in the list
  const hasSampleCases = cases.some(
    c => c.title === 'Sample Property Dispute' && c.description === 'Automated sample case created via seeder.'
  );
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-4 sm:p-6 bg-slate-50 dark:bg-slate-950 min-h-screen">
      ...
      {/* Duplicate Cleanup Banner — only shown when sample cases detected */}
      {hasSampleCases && (
        <div className="flex items-center justify-between gap-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-xl px-5 py-3">
          <div className="flex items-center gap-3">
            <svg className="w-5 h-5 text-amber-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" />
            </svg>
            <p className="text-sm font-semibold text-amber-800 dark:text-amber-300">
              Duplicate sample cases detected. These were auto-generated by a previous bug and are not real cases.
            </p>
          </div>
          <button
            onClick={handleClearSamples}
            className="flex-shrink-0 inline-flex items-center gap-1.5 px-4 py-1.5 bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold rounded-lg transition-colors"
          >
                  ...
                  <tr key={item._id || item.id} className="hover:bg-slate-50/80 transition-colors group">
                    ...
                    <td className="px-6 py-5">
                       <div className="flex flex-col gap-1.5">
                         {/* Client — always show if name is available */}
                         {(item.user?.name || item.user_name) && (
                           <div className="flex items-center gap-2">
                             ...
                             <span className="font-semibold text-slate-700 dark:text-slate-300">{item.user?.name || item.user_name}</span>
                           </div>
                         )}
                         {/* Opponent — only show if value exists */}
                         {item.opponent_name && (
                           <div className="flex items-center gap-2">
                             <span className="text-[10px] bg-red-50 text-red-600 px-1.5 py-0.5 rounded font-bold uppercase">Opponent</span>
                             <span className="font-semibold text-slate-700 dark:text-slate-300">{item.opponent_name}</span>
                           </div>
                         )}
                         {/* Opponent Lawyer — only show if value exists */}
                         {item.opponent_lawyer && (
                           <div className="flex items-center gap-2">
                             ...
                             <span className="text-slate-500 dark:text-slate-400">{item.opponent_lawyer}</span>
                           </div>
                         )}
                         {/* Fallback when nothing is filled */}
                         {!item.user?.name && !item.user_name && !item.opponent_name && !item.opponent_lawyer && (
                    ...
                    <td className="px-6 py-5 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          onClick={() => handleDeleteCase(item._id || item.id, item.title)}
                          disabled={deletingCaseId === (item._id || item.id)}
                          className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all"
                          title="Permanent Delete"
                        >
                          {deletingCaseId === (item._id || item.id) ? (
