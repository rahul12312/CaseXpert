...
const CaseIntelligenceReport = () => {
    ...
    if (loading) return <div className="p-10 text-center text-slate-500 dark:text-slate-400">Loading Intelligence Report...</div>;

    // If no report exists, show "Generate" CTA
    if (!data || !data.report) {
        return (
            <div className="max-w-4xl mx-auto p-8 text-center bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 mt-10">
                <div className="mx-auto w-24 h-24 bg-purple-50 rounded-full flex items-center justify-center text-4xl mb-6">
                    ✨
                </div>
                <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">No Intelligence Report Found</h2>
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <div className="flex items-center gap-3">
                        <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Case Intelligence Report</h1>
                        <span className="bg-purple-100 text-purple-700 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide">
                            AI Generated
                        </span>
                    </div>
                    <p className="text-slate-500 dark:text-slate-400 mt-1">Generated on {new Date(generated_at).toLocaleDateString()} at {new Date(generated_at).toLocaleTimeString()}</p>
                </div>
                <div className="flex gap-3">
                    <button
                        onClick={() => window.print()}
                        className="px-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-medium hover:bg-slate-50 dark:bg-slate-950"
                    >
                        🖨️ Print / PDF
                    </button>
                    <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                {/* Risk Score Card */}
                <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 relative overflow-hidden">
                    <h3 className="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-4">Risk Assessment</h3>
                ...
                {/* Prediction Card */}
                <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col justify-between bg-gradient-to-br from-white to-indigo-50/50">
                    <div>
                        <h3 className="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Outcome Prediction</h3>
                ...
                <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                    <h3 className="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-4">Case Velocity</h3>
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4 pt-2">
                            <div className="bg-slate-50 dark:bg-slate-950 p-3 rounded-lg text-center">
                                <span className="block text-xl font-bold text-slate-800 dark:text-slate-200">{stats?.total_hearings || 0}</span>
                                <span className="text-xs text-slate-500 dark:text-slate-400">Hearings</span>
                            </div>
                            <div className="bg-slate-50 dark:bg-slate-950 p-3 rounded-lg text-center">
                                <span className="block text-xl font-bold text-slate-800 dark:text-slate-200">{stats?.total_documents || 0}</span>
                                <span className="text-xs text-slate-500 dark:text-slate-400">Documents</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                        ...
                        {report.key_legal_issues && (
                            <div className="mt-6">
                                <h4 className="text-sm font-semibold text-slate-900 dark:text-white mb-3">Key Legal Issues Identified</h4>
                    ...
                    {/* SWOT Analysis Grid */}
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-6">SWOT Analysis</h3>
                ...
                <div className="space-y-8">
                    ...
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="font-bold text-slate-900 dark:text-white">Document Health</h3>
                        ...
                        {report.document_analysis?.missing_critical_documents?.length > 0 ? (
                            ...
                            <div className="text-sm text-slate-500 dark:text-slate-400">All critical documents appear to be present.</div>
                        )}
                    ...
                    {/* Hearing Insights */}
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                        <h3 className="font-bold text-slate-900 dark:text-white mb-4">Hearing Trends</h3>
                        <div className="space-y-4">
                            <div>
                                <span className="text-xs text-slate-500 dark:text-slate-400 uppercase">Adjournment Risk</span>
                            ...
                            <div>
                                <span className="text-xs text-slate-500 dark:text-slate-400 uppercase">Next Likely Step</span>
                                <p className="text-sm font-medium text-slate-800 dark:text-slate-200">{report.hearing_insights?.next_likely_step}</p>
                            </div>
                        </div>
                    </div>
