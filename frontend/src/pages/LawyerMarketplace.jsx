            ...
            <div className="bg-gradient-to-r from-blue-700 to-blue-900 text-white py-12 relative overflow-hidden">
                ...
                <div className="container mx-auto px-4 relative">
                    ...
                    <div className="max-w-5xl mx-auto">
                        <div className="bg-white rounded-2xl shadow-2xl p-2 flex flex-col md:flex-row items-center gap-2 border-4 border-white/20">

                            {/* AI Matcher Section (Left Side & Expandable) */}
                            <div className={`transition-all duration-500 ease-in-out w-full md:w-auto ${isAiExpanded ? 'flex-[1.5] bg-slate-50 rounded-xl p-1 shadow-inner border border-slate-200' : ''}`}>
                                {!isAiExpanded ? (
                                    <button
                                    ...
                                    <div className="flex flex-col sm:flex-row items-center w-full gap-2 p-1 animate-in fade-in slide-in-from-left-4 duration-300">
                                        <div className="flex items-center w-full gap-2">
                                            ...
                                            <input
                                                ...
                                                className="flex-1 bg-transparent text-slate-900 px-2 py-2 sm:py-3 focus:outline-none font-medium placeholder:text-slate-400 text-sm sm:text-base min-w-0"
                                            />
                                        </div>
                                        <div className="flex w-full sm:w-auto gap-1 border-t sm:border-t-0 pt-2 sm:pt-0 border-slate-200">
                                            <button
                                            ...
                                            <button
                                                ...
                                                className="p-2 sm:p-2.5 text-slate-400 hover:text-slate-600 rounded-lg shrink-0"
                                            >
                                                <X className="w-5 h-5" />
                                            </button>
                                        </div>
                            ...
                            <div className="flex-1 flex items-center relative group w-full">
                                ...
                                <input
                                    ...
                                    className="w-full pl-12 pr-4 py-3 sm:py-4 rounded-xl text-slate-900 font-medium text-base sm:text-lg focus:outline-none placeholder:text-slate-400"
                                />
                                {searchQuery && (
                                    <button
                                        onClick={() => { setSearchQuery(''); handleSearch(); }}
                                        className="absolute right-4 p-1 hover:bg-slate-100 rounded-full text-slate-400"
                                    >
                                        <X className="w-4 h-4" />
                                    </button>
                                )}
                ...
                <div className="container mx-auto px-4 -mt-8 relative z-10 animate-in fade-in slide-in-from-top-4 duration-500">
                    <div className="max-w-5xl mx-auto bg-amber-50 border-2 border-amber-200 rounded-2xl p-4 sm:p-5 shadow-xl flex flex-col sm:flex-row items-center gap-4">
                        ...
                        <button
                            ...
                            className="w-full sm:w-auto bg-white/80 hover:bg-white text-amber-800 px-4 py-2 rounded-xl text-xs sm:text-sm font-bold border border-amber-200 transition-all whitespace-nowrap"
                        >
                            Reset AI
                        </button>
                    </div>
                    ...
                    <main className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-8">
                            <div className="text-center sm:text-left">
                                ...
                                <p className="text-slate-500 text-xs sm:text-sm mt-1">
                                    Showing <span className="text-slate-900 font-semibold">{pagination.total}</span> verified legal experts
                                </p>
                            </div>
                            ...
                            <div className="flex items-center gap-3 w-full sm:w-auto">
                                <button
                                    ...
                                    className="lg:hidden flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl font-bold shadow-sm active:bg-slate-50"
                                >
                                    <Filter className="w-4 h-4" />
                                    Filters
                                </button>
                                <div className="flex-1 sm:flex-none flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-3 py-2 sm:py-1.5 shadow-sm">
                                    <SlidersHorizontal className="w-4 h-4 text-slate-400" />
                                    <select
                                        value={sortBy}
                                        onChange={(e) => setSortBy(e.target.value)}
                                        className="bg-transparent text-slate-700 text-xs sm:text-sm font-bold focus:outline-none w-full"
                                    >
                        ...
                        {showFilters && (
                            <div className="lg:hidden fixed inset-0 z-[60] animate-in fade-in duration-300">
                                ...
                                <div className="absolute right-0 top-0 bottom-0 w-[85%] max-w-sm bg-white shadow-2xl animate-in slide-in-from-right duration-500">
                                    <div className="p-4 border-b border-slate-100 flex items-center justify-between">
                                        <h3 className="font-bold text-slate-900">Filter Search</h3>
                                        <button onClick={() => setShowFilters(false)} className="p-2 bg-slate-100 rounded-lg">
                        ...
                        {loading ? (
                            <div className="flex flex-col items-center justify-center py-24 bg-white rounded-3xl border border-slate-100 shadow-sm">
                                ...
                                <p className="text-slate-500 font-medium">Fetching best lawyers...</p>
                            </div>
                        ) : lawyers.length === 0 ? (
                            <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-16 text-center">
                                ...
                                <h3 className="text-2xl font-bold text-slate-900 mb-2">No Matching Experts</h3>
                                ...
                                {pagination.pages > 1 && !aiResults && (
                                    <div className="mt-12 flex items-center justify-center gap-2 sm:gap-3">
                                        <button
                                            ...
                                            className="p-2 sm:p-3 border border-slate-200 rounded-xl hover:bg-white disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                                        >
