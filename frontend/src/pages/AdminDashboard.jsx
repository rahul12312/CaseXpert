    ...
    <div className="min-h-screen bg-gray-50 p-6 relative">
      ...
      {viewLoading && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/10 backdrop-blur-[2px]">
          <div className="bg-white p-3 rounded-full shadow-xl shadow-primary-200/50">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {stats && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div className="bg-white p-6 rounded-xl shadow-sm border-l-4 border-blue-500">
              <h3 className="text-gray-500 text-xs font-bold uppercase tracking-wider">Total Lawyers</h3>
              <p className="text-3xl font-bold text-gray-900 mt-2">{stats.lawyers.total}</p>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border-l-4 border-green-500">
              <h3 className="text-gray-500 text-xs font-bold uppercase tracking-wider">Active Cases</h3>
              <p className="text-3xl font-bold text-gray-900 mt-2">{stats.cases.active}</p>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border-l-4 border-yellow-500">
              ...
              <p className="text-3xl font-bold text-gray-900 mt-2">{stats.lawyers.pending}</p>
              <div className="mt-2 text-xs font-medium bg-yellow-50 text-yellow-700 px-2 py-1 rounded-md inline-block">
                Action Required
              </div>
            </div>
            <div className="bg-white p-6 rounded-xl shadow-sm border-l-4 border-purple-500">
              <h3 className="text-gray-500 text-xs font-bold uppercase tracking-wider">Platform Health</h3>
              <p className="text-3xl font-bold text-gray-900 mt-2">{stats.cases.total}</p>
              <div className="mt-2 text-xs font-medium bg-purple-50 text-purple-700 px-2 py-1 rounded-md inline-block">
                Total Cases Managed
              </div>
            </div>
          </div>
        ...
        <div className="flex border-b border-gray-200 mb-6 overflow-x-auto">
          <button
            ...
            className={`px-6 py-3 font-medium text-sm whitespace-nowrap flex items-center gap-2 ${tab === 'LAWYERS' ? 'border-b-2 border-primary-600 text-primary-600' : 'text-gray-500 transition-colors hover:text-gray-700'}`}
          >
            <User className="w-4 h-4" /> Lawyer Management
          </button>
          <button
            onClick={() => setTab('CASES')}
            className={`px-6 py-3 font-medium text-sm whitespace-nowrap flex items-center gap-2 ${tab === 'CASES' ? 'border-b-2 border-primary-600 text-primary-600' : 'text-gray-500 transition-colors hover:text-gray-700'}`}
          >
            <BookOpen className="w-4 h-4" /> Case Oversight
          </button>
          <button
            onClick={() => setTab('REPORT')}
            className={`px-6 py-3 font-medium text-sm whitespace-nowrap flex items-center gap-2 ${tab === 'REPORT' ? 'border-b-2 border-primary-600 text-primary-600' : 'text-gray-500 transition-colors hover:text-gray-700'}`}
          >
          ...
          <div className="bg-white shadow-sm rounded-xl overflow-hidden border border-gray-200">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Lawyer Profile</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredLawyers.map(l => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2.5 py-1 inline-flex text-xs leading-none font-semibold rounded-full border ${l.verification_status === 'VERIFIED' ? 'bg-green-50 text-green-700 border-green-200' :
                          l.verification_status === 'REJECTED' ? 'bg-red-50 text-red-700 border-red-200' :
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-700">{l.experience} Years</div>
                        <div className="text-xs text-gray-500 truncate max-w-[150px]">{l.specialization}</div>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-primary-700 bg-primary-50 rounded-lg hover:bg-primary-100 transition-colors border border-primary-200"
                        >
            ...
            {filteredLawyers.length === 0 && (
              <div className="p-12 text-center">
                ...
                <h3 className="text-gray-900 font-medium">No lawyers found</h3>
                <p className="text-gray-500 text-sm mt-1">Try adjusting your status filter.</p>
              </div>
            )}
          </div>
        )}
        ...
        {tab === 'CASES' && (
          <div className="bg-white shadow rounded-lg overflow-hidden">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Assigned Lawyer</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {cases.map(c => (
                  <tr key={c.id}>
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500">{c.client_name}</td>
                    <td className="px-6 py-4 text-sm text-gray-500">{c.lawyer_name || <span className="text-orange-500 italic">Unassigned</span>}</td>
                    <td className="px-6 py-4 text-sm">
                      <span className="px-2 py-1 text-xs rounded bg-gray-100 font-medium text-gray-700">{c.status}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            {cases.length === 0 && (
              <div className="p-8 text-center text-gray-500 text-sm">No cases found in the system.</div>
            )}
          </div>
        )}
        ...
        {tab === 'REPORT' && (
          <div className="bg-white shadow rounded-lg p-6">
            <h3 className="font-bold text-gray-900 mb-4">Top Lawyers by Caseload</h3>
            <div className="space-y-4">
              {stats?.topLawyers?.map((tl, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 rounded">
                  <div>
                    <p className="font-medium text-gray-900">{tl.lawyer_name}</p>
                    <p className="text-xs text-gray-500">{tl.specialization}</p>
                  </div>
      ...
      {selectedLawyer && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
              ...
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className={`px-2.5 py-0.5 text-xs font-bold rounded-full uppercase tracking-wide border ${selectedLawyer.verification_status === 'VERIFIED' ? 'bg-green-100 text-green-700 border-green-200' :
                      selectedLawyer.verification_status === 'REJECTED' ? 'bg-red-100 text-red-700 border-red-200' :
                        'bg-yellow-100 text-yellow-700 border-yellow-200'
                      }`}>
                      {selectedLawyer.verification_status.replace('_', ' ')}
                    </span>
                    <span className="text-sm text-gray-500">• Joined {new Date(selectedLawyer.created_at).toLocaleDateString()}</span>
                  </div>
                </div>
              </div>
              <button
                    ...
                    <div className="bg-gray-50 rounded-xl p-4 space-y-3 border border-gray-100">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-medium text-gray-900">{selectedLawyer.email}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-medium text-gray-900">{selectedLawyer.phone || 'Not provided'}</p>
                        </div>
                      </div>
                    </div>
                  </section>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                          ...
                          <p className="text-lg font-bold text-gray-900">{selectedLawyer.experience} Years</p>
                        </div>
                        <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                          <p className="text-xs text-gray-500">Fees</p>
                          <p className="text-lg font-bold text-gray-900">₹{selectedLawyer.consultation_fee || '0'} /hr</p>
                        </div>
                      </div>

                      <div>
                        <p className="text-sm text-gray-500 mb-1">Languages</p>
                        <p className="text-sm font-medium text-gray-900">{selectedLawyer.languages || 'Not specified'}</p>
                      </div>
                    </div>
                  </section>
                </div>
                    ...
                    {selectedLawyer.documents && selectedLawyer.documents.length > 0 ? (
                      <div className="space-y-3">
                        {selectedLawyer.documents.map((doc, idx) => (
                          <div key={idx} className="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-lg hover:border-primary-300 hover:shadow-sm transition-all group">
                            <div className="flex items-center gap-3 overflow-hidden">
                              ...
                              <div className="min-w-0">
                                <p className="text-sm font-medium text-gray-900 truncate">{doc.title || doc.document_type || 'Document'}</p>
                                <p className="text-xs text-gray-500">{doc.file_type?.toUpperCase()} • {(doc.file_size / 1024).toFixed(0)} KB</p>
                              </div>
                            </div>
                            <button
                  ...
                  {selectedLawyer.bio && (
                    <section>
                      ...
                      <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                        <p className="text-sm text-gray-600 leading-relaxed italic">"{selectedLawyer.bio}"</p>
                      </div>
                    </section>
                  )}
                </div>
            ...
            <div className="p-4 border-t border-gray-100 bg-gray-50 flex flex-col sm:flex-row justify-end gap-3 rounded-b-2xl">
              <button
                onClick={() => setSelectedLawyer(null)}
