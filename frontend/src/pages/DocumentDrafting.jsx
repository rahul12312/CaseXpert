        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="space-y-6">
            <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 shadow-sm">
              <h2 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">Document Details</h2>
              <form onSubmit={handleGenerate} className="space-y-4">
                <div className="space-y-2">
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300" htmlFor="documentType">
                    Document Type
                  </label>
                  <select
                    id="documentType"
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300" htmlFor="details">
          ...
          <div className="space-y-6">
            <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 shadow-sm h-full flex flex-col">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Editor / Preview</h2>
                {draft && (
                  <div className="flex gap-2">
                    <button onClick={downloadDraft} className="flex items-center gap-2 px-3 py-1.5 text-sm bg-slate-100 dark:bg-slate-800 rounded-lg hover:bg-slate-200">
              ...
              <div className="flex-1 relative rounded-lg bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 min-h-[500px]">
                <textarea
                  value={draft}
                  onChange={(e) => setDraft(e.target.value)}
                  placeholder="Draft content will appear here..."
                  className="w-full h-full p-6 bg-transparent border-none focus:ring-0 resize-none font-mono text-sm leading-relaxed text-slate-800 dark:text-slate-200"
                  spellCheck="false"
                />
                {!draft && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none opacity-50">
                    <FileText className="w-16 h-16 text-slate-300 mb-4" />
                    <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Document Workspace</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="space-y-8">
          ...
          {categories.map(category => (
            <div key={category} className="space-y-4">
              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200 border-l-4 border-blue-500 pl-3">{category}</h3>
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {legalTemplates.filter(t => t.category === category).map(template => (
                  <div key={template.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-5 hover:shadow-lg transition-all hover:border-blue-300 flex flex-col">
                    ...
                    <h4 className="font-bold text-slate-900 dark:text-white mb-2">{template.title}</h4>
                    <p className="text-sm text-slate-500 dark:text-slate-400 mb-4 flex-1">{template.description}</p>
                    <button
                      onClick={() => handleUseTemplate(template)}
                      className="w-full py-2 bg-white dark:bg-slate-900 border border-blue-600 text-blue-600 font-medium rounded-lg hover:bg-blue-600 hover:text-white transition-colors flex items-center justify-center gap-2"
                    >
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
              <div>
                <h2 className="text-xl font-bold text-slate-900 dark:text-white">Your Document Vault</h2>
                <p className="text-sm text-slate-500 dark:text-slate-400">Secure cloud storage for your legal files (Only accessible by you)</p>
              </div>
            ...
            {!storageLoading && storedDocs.length === 0 && (
              <div className="text-center py-12 bg-slate-50 dark:bg-slate-950 rounded-lg dashed-border">
                <HardDrive className="w-12 h-12 text-slate-300 mx-auto mb-3" />
                <p className="text-slate-500 dark:text-slate-400">Your vault is empty. Draft new documents or upload existing ones.</p>
              </div>
            )}
            ...
            {!storageLoading && storedDocs.length > 0 && (
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                  ...
                  <tbody>
                    {storedDocs.map(doc => (
                      <tr key={doc.id} className="bg-white dark:bg-slate-900 border-b hover:bg-slate-50 dark:bg-slate-950">
