            ...
            try {
                ...
                if (data.success) {
                    setFormData({
                        ...
                        // Lawyer fields
                        specialization: data.user.lawyer_profile?.specialization || '',
                        experience: data.user.lawyer_profile?.experience || '',
                        consultation_fee: data.user.lawyer_profile?.consultation_fee || '',
                        bio: data.user.lawyer_profile?.bio || '',
                        lawyer_city: data.user.lawyer_profile?.city || '',
                        lawyer_state: data.user.lawyer_profile?.state || '',
                        languages: Array.isArray(data.user.lawyer_profile?.languages) ? data.user.lawyer_profile.languages.join(', ') : data.user.lawyer_profile?.languages || '',
                        bar_council_id: data.user.lawyer_profile?.bar_council_id || '',
                        bar_council_state: data.user.lawyer_profile?.bar_council_state || '',
                        enrollment_year: data.user.lawyer_profile?.enrollment_year || ''
                    });
                ...
                <div className="lg:col-span-2 space-y-6">
                                    ...
                                    <div className="rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 p-4">
                                        {formData.identity_proof_url ? (
                                            ...
                                            <div className="space-y-3">
                                                <a href="https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf" target="_blank" rel="noreferrer" className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                    ...
                    {formData.role === 'lawyer' && (
                        <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
                            <div className="border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/20 px-6 py-4">
                                <h3 className="font-semibold text-slate-900 dark:text-white flex items-center gap-2">
                                    <span className="flex h-5 w-5 items-center justify-center rounded bg-blue-100 dark:bg-blue-900/30 text-blue-600">⚖️</span> 
                                    Professional Information
                                </h3>
                            </div>
                            <div className="p-6 space-y-6">
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                    <InputField label="Specialization" name="specialization" formData={formData} handleChange={handleChange} isEditing={isEditing} />
                                    <InputField label="Experience (Years)" name="experience" type="number" formData={formData} handleChange={handleChange} isEditing={isEditing} />
                                    <InputField label="Consultation Fee" name="consultation_fee" type="number" formData={formData} handleChange={handleChange} isEditing={isEditing} />
                                    <InputField label="Languages Spoken (comma separated)" name="languages" formData={formData} handleChange={handleChange} isEditing={isEditing} />
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                    <InputField label="Bar Council ID" name="bar_council_id" formData={formData} handleChange={handleChange} isEditing={isEditing} />
                                    <InputField label="Bar Council State" name="bar_council_state" formData={formData} handleChange={handleChange} isEditing={isEditing} />
                                    <InputField label="Enrollment Year" name="enrollment_year" type="number" formData={formData} handleChange={handleChange} isEditing={isEditing} />
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                    <InputField label="Practice City" name="lawyer_city" formData={formData} handleChange={handleChange} isEditing={isEditing} />
                                    <InputField label="Practice State" name="lawyer_state" formData={formData} handleChange={handleChange} isEditing={isEditing} />
                                </div>
                                <div>
                                    <label className="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">
                                        Professional Bio
                                    ...
                                    {isEditing ? (
                                        <textarea
                                            ...
                                            rows="4"
                                            className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none dark:text-white"
                                        />
                                    ) : (
                                        <div className="rounded-lg border border-transparent bg-slate-50 dark:bg-slate-800/50 px-3 py-2 text-sm text-slate-900 dark:text-white whitespace-pre-wrap">
                                            {formData.bio || <span className="text-slate-400 italic">Not provided</span>}
                                        </div>
                                    )}
                                </div>
                            </div>
