...
const GenerateCaseReport = () => {
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 dark:bg-slate-950 text-center px-4">
            ...
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Generating Advanced Analysis</h2>
            <p className="text-slate-500 dark:text-slate-400 max-w-md">Our AI is analyzing 500+ data points, reviewing documents, and predicting outcomes based on precedents...</p>
    ...
    return (
        <div className="max-w-4xl mx-auto p-6">
            <div className="mb-8">
                <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Advanced Case Report Wizard</h1>
                <p className="text-slate-500 dark:text-slate-400">Generate a comprehensive AI-powered legal analysis.</p>
            </div>
            ...
            <div className="flex items-center justify-between mb-8 relative">
                ...
                {[1, 2, 3, 4].map((s) => (
                    <div key={s} className={`flex flex-col items-center bg-slate-50 dark:bg-slate-950 px-2`}>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm transition-colors ${step >= s ? 'bg-purple-600 text-white' : 'bg-slate-300 text-slate-600 dark:text-slate-400'
                            }`}>
                            {s}
                        </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 md:p-8">
                {step === 1 && (
                    <div className="space-y-6">
                        <h2 className="text-xl font-semibold text-slate-800 dark:text-slate-200 border-b pb-2">Step 1: Case Details</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Case Number</label>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Current Stage</label>
                ...
                {step === 2 && (
                    <div className="space-y-6">
                        <h2 className="text-xl font-semibold text-slate-800 dark:text-slate-200 border-b pb-2">Step 2: Parties Involved</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="md:col-span-2">
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Plaintiff / Complainant Details</label>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Opponent Lawyer</label>
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Defendant / Respondent Details</label>
                ...
                {step === 3 && (
                    <div className="space-y-6">
                        <h2 className="text-xl font-semibold text-slate-800 dark:text-slate-200 border-b pb-2">Step 3: Case Summary & Issues</h2>
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Short Case Description</label>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Key Issues Involved</label>
                            ...
                            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Separate issues with commas.</p>
                        </div>
                    </div>
                )}
                ...
                {step === 4 && (
                    <div className="space-y-6">
                        <h2 className="text-xl font-semibold text-slate-800 dark:text-slate-200 border-b pb-2">Step 4: Evidence & Documents</h2>
                        <div className="bg-slate-50 dark:bg-slate-950 p-6 rounded-lg border-2 border-dashed border-slate-300 text-center">
                            <div className="mb-4 text-4xl">📂</div>
                            <h3 className="text-sm font-medium text-slate-900 dark:text-white">Upload Case Files</h3>
                            <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">PDF, DOCX, JPG (Max 10MB)</p>
                            <input
                                type="file"
                                multiple
                                onChange={handleFileChange}
                                className="block w-full text-sm text-slate-500 dark:text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-purple-50 file:text-purple-700 hover:file:bg-purple-100"
                            />
                        </div>

                        {documents.length > 0 && (
                            <div className="mt-4">
                                <h4 className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Selected Files:</h4>
                                <ul className="space-y-2">
                                    {documents.map((doc, idx) => (
                                        <li key={idx} className="flex items-center justify-between bg-white dark:bg-slate-900 p-2 rounded border border-slate-200 dark:border-slate-700 text-sm">
                ...
                <div className="mt-8 flex justify-between pt-6 border-t border-slate-100 dark:border-slate-800">
                    <button
                        ...
                        className={`px-6 py-2 rounded-lg text-sm font-medium ${step === 1 ? 'text-slate-300 cursor-not-allowed' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:bg-slate-800'}`}
                    >
                        Back
                    </button>
