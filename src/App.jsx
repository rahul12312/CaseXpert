...
import AdminDashboardNew from './pages/admin/Dashboard.jsx';
import AdminLawyers from './pages/admin/Lawyers.jsx';
...
// (Legacy Admin Dashboard kept just in case)
import AdminDashboardLegacy from './pages/AdminDashboard.jsx';
...
const App = () => {
  ...
  const isFeaturePage = 
    ...
    location.pathname.startsWith('/lawyer/accepted-cases') ||
    location.pathname.startsWith('/lawyer/case-requests') ||
    location.pathname.startsWith('/lawyer/client-queries') ||
    location.pathname.startsWith('/admin') ||
    location.pathname === '/dashboard';

  const hideFooter = isFeaturePage;
  const hideNavbar = location.pathname.startsWith('/consultation') || location.pathname.startsWith('/admin');
  const fullScreen = location.pathname.startsWith('/assistant') || location.pathname.startsWith('/consultation') || location.pathname === '/dashboard' || location.pathname === '/profile' || location.pathname.startsWith('/admin');
    ...
    <div className={`flex min-h-screen flex-col bg-slate-50 dark:bg-slate-950 overflow-x-hidden`}>
      ...
      <main className={`flex flex-1 flex-col ${!fullScreen ? 'pt-16 md:pt-24 pb-8 sm:px-6 lg:px-8 mx-auto w-full max-w-7xl px-4' : 'pt-0 w-full px-0 overflow-hidden'}`}>
        ...
        <Routes>
          ...
          />
          <Route 
            path="/lawyer/client-queries" 
            element={<ProtectedRoute allowedRoles={['lawyer']}><LawyerClientQueries /></ProtectedRoute>} 
          />
