...
const CaseDocuments = ({ caseId, initialDocuments = [], onRefresh }) => {
    ...
    return (
        <div className="space-y-4 sm:space-y-6">
            ...
            {/* Action Bar */}
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 bg-white dark:bg-slate-900 p-3 sm:p-4 rounded-lg sm:rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                <div className="relative w-full sm:flex-1 sm:max-w-md">
                    ...
                    <input
                        ...
                        className="w-full pl-9 sm:pl-10 pr-4 py-2 sm:py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg text-xs sm:text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none"
                    />
                </div>
            ...
            {/* Document Grid/List */}
            <div className="bg-white dark:bg-slate-900 rounded-lg sm:rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                {filteredDocuments.length === 0 ? (
                    <div className="py-12 sm:py-20 px-4 text-center space-y-3 sm:space-y-4">
                        <div className="w-16 h-16 sm:w-20 sm:h-20 bg-slate-50 dark:bg-slate-950 rounded-full flex items-center justify-center mx-auto">
                            <FileText className="w-8 h-8 sm:w-10 sm:h-10 text-slate-300" />
                        </div>
                        <div className="max-w-xs mx-auto">
                            <h4 className="font-semibold text-slate-900 dark:text-white text-sm sm:text-base">No documents found</h4>
                                ...
                                <div className="block sm:hidden space-y-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            {isRenaming === doc.id ? (
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <button onClick={() => setIsRenaming(null)} className="p-1 text-slate-400 hover:bg-slate-100 dark:bg-slate-800 rounded"><X className="w-4 h-4" /></button>
                                                </div>
                                            ) : (
                                                <div>
                                                    <h4 className="text-xs font-medium text-slate-900 dark:text-white truncate flex items-center gap-2">
                                                        {doc.original_name}
                                                        {doc.file_type === 'pdf' && (
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <button
                                            ...
                                            className="flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-medium text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-950 hover:bg-slate-100 dark:bg-slate-800 rounded-lg transition-all"
                                        >
                                ...
                                <div className="hidden sm:flex items-center gap-4">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        {isRenaming === doc.id ? (
                                            ...
                                            <h4 className="text-sm font-medium text-slate-900 dark:text-white truncate flex items-center gap-2">
                                                {doc.original_name}
                                                {doc.file_type === 'pdf' && (
                                    ...
                                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button
                                            onClick={() => window.open(doc.file_url, '_blank')}
                                            title="View"
                                            className="p-2 text-slate-500 dark:text-slate-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-all"
                                        >
                                        ...
                                        <button
                                            onClick={() => handleDelete(doc.id)}
                                            title="Delete"
                                            className="p-2 text-slate-500 dark:text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all"
                                        >
                                ...
                                {summary && !isSummarizing && (
                                    <div className="mt-4 p-3 sm:p-4 bg-violet-50/50 border border-violet-100 rounded-lg sm:rounded-xl relative">
                                        ...
                                        <div className="flex gap-3">
                                            <div className="p-2 bg-white dark:bg-slate-900 rounded-lg shadow-sm h-fit">
