...
exports.getSample = async (req, res) => {
    try {
        ...
        console.log('\n📄 GET SAMPLE METADATA REQUEST');
        console.log('   Document Type:', type);

        const [rows] = await db.execute('SELECT * FROM document_samples WHERE document_type = ?', [type]);

        if (rows.length === 0) {
            // Check fallback in hardcoded DOCUMENT_TYPES if not in DB
            const fallback = DOCUMENT_TYPES[type];
            if (!fallback) {
            ...
            return res.json({
                success: true,
                isGenerated: true,
                message: "Sample not found in managed storage, using generated fallback",
                type: type,
                title: fallback.name,
                url: `/api/documents/samples/${type}/view` // Internal route for on-the-fly generation
            });
        }
        ...
        res.json({
            ...
            sample: {
                ...
                url: sample.sample_pdf_url, // Final accessible URL /public/samples/...
                fullUrl: `${req.protocol}://${req.get('host')}${sample.sample_pdf_url}`
            }
        });
        ...
        res.status(500).json({
            success: false,
            message: 'Failed to fetch sample information'
        });
    }
};
...
exports.getDocumentSamples = async (req, res) => {
    try {
        const db = getDb();
        const [rows] = await db.execute('SELECT * FROM document_samples ORDER BY title ASC');
        ...
        const samples = rows.map(row => ({
            ...
            url: row.sample_pdf_url,
            fullUrl: `${req.protocol}://${req.get('host')}${row.sample_pdf_url}`
        }));
        ...
        res.status(500).json({ success: false, message: 'Failed to fetch document samples' });
    }
};
